...
def print_result_row(result_specification):
    global headers_printed
    if not headers_printed:
        headers = map(lambda t: t[0], result_specification)
        print ", ".join(map(lambda h: h.rjust(10, ' '), headers))
        headers_printed = True

    row_format_string = ", ".join(map(lambda t: t[1], result_specification))
    data = map(lambda t: t[2], result_specification)

    print row_format_string % tuple(data)


def run_simulation(fuzz_classes, plan, team_size, max_clock_tick, concentration):
    ...
    print_result_row(result_specification)
...
for experimental_parameter in experimental_parameters:
    try:
        run_simulation(*experimental_parameter)
    except Exception as e:
        print >> sys.stderr, e, "encountered during run of", str(experimental_parameter), "."
