...
for workflow in ['TestDriven', 'WaterfallD']:
    ...
    for project_size in sorted(project_sizes('large').values())[1:]:

        x_values, y_values = get_time_series(
            x_field=lambda r: r.fuzz_total,
            y_field=lambda r: r.mtf_avg,
            row_filter=lambda r:
                r.workflow == workflow and r.project_size == project_size and r.project_type == 'large')

        d = 1 - project_size/ (1.0 * max(project_sizes('large').values()))
...
plt.savefig('mtf_against_total_fuzz_large_projects.pgf', bbox_inches='tight')
