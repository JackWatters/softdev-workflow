...
class Chunk:
    ...
    def _add_dependencies(self, my_random, candidate_chunks, probability):
        for candidate in sorted(candidate_chunks, key=lambda c: c.id):
            p = my_random.random()
            if p <= probability:
    ...
    def _insert_bugs(self, random):
        while random.random() <= self.probability_new_bug:
            self._bugs.add(Bug(self))


    def refactor (self, random):
        to_remove = set()
        for existing_chunk in sorted(list(self.dependencies), key=lambda d : d.id ):
            p = random.random()
    ...
    def operate(self, random):
        for bug in sorted(self.bugs_in_dependencies.union(self.bugs), key=lambda b : b.id ):
            bug.manifest(random)


    @property
    def bugs(self):
        return sorted(list(self._bugs), key=lambda b : b.id)
